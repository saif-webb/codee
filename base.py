...
def extract_answer_from_response(response):
    """Extract the most relevant answer from the search response."""
    if response is None:
        return None
    ...
    if result:
        ...
        if snippet_tag:
            return snippet_tag.get_text(strip=True)

    return None

def search_website(query, base_url):
    """Search a specific website for the query."""
    encoded_query = urllib.parse.quote_plus(query)
    url = f"https://www.bing.com/search?q=site:{base_url}+{encoded_query}"
    response = safe_request(url)
    if response and not detect_bot_block(response):
...
def bing_search(query):
    """Perform a Bing search with prioritized rules."""
    # Extract company name from the query (basic implementation, can be enhanced)
    company_name = query.split()[0].lower()

    # 1st Priority: Search the official website
    official_website_answer = search_website(query, f"{company_name}.com")
    if official_website_answer:
        return official_website_answer

    # 2nd Priority: Search popular company-related websites (LinkedIn, Justdial)
    for site in ["linkedin.com", "justdial.com"]:
        answer = search_website(query, site)
        if answer:
            return answer

    # 3rd Priority: General company-based search
    general_search_answer = search_website(query, company_name)
    if general_search_answer:
        return general_search_answer
